...
export default function ProductPage() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-[family-name:var(--font-poppins)] pb-24 md:pb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                    ...
                    <div className="md:col-span-6 lg:col-span-5 flex flex-col pt-2 md:pt-0">
                        <div className="bg-white md:rounded-[2.5rem] p-4 sm:p-6 lg:p-10 md:shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50 relative">
                            ...
                            <div className="absolute top-4 right-4 flex items-center z-20 md:hidden">
                                <button
                                    onClick={handleShare}
                                    className="px-3 py-1.5 bg-white/95 backdrop-blur-sm shadow-sm rounded-xl flex items-center space-x-1.5 text-gray-600 font-medium text-xs active:scale-95 transition-all border border-gray-50"
                                >
                    ...
                    <div className="md:col-span-6 lg:col-span-7 space-y-6 px-4 md:px-0">
                        ...
                        <div className="bg-white p-6 rounded-[2rem] border border-gray-100/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mt-2 md:mt-0">
                            ...
                            <div className="mt-4 grid grid-cols-3 gap-2 py-3 border-t border-gray-50">
                                <div className="text-center">
                                    <span className="block text-[10px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">Kondisi</span>
                                    <span className="text-xs sm:text-sm font-bold text-gray-900">{product.condition === 'NEW' ? 'Baru' : 'Bekas'}</span>
                                </div>
                                <div className="text-center border-x border-gray-50">
                                    <span className="block text-[10px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">Berat</span>
                                    <span className="text-xs sm:text-sm font-bold text-gray-900">{product.weight} gr</span>
                                </div>
                                <div className="text-center">
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center space-x-1.5">
                                        <h3 className="font-semibold text-xs sm:text-base text-gray-900 truncate">{product.shop.name}</h3>
                                        <div className="bg-green-100 px-1.5 py-0.5 rounded text-[9px] font-medium text-green-700">Official</div>
                                    </div>
                                ...
                                <button className="px-3 py-1.5 border border-[#1B5E20] text-[#1B5E20] text-xs sm:text-sm font-medium rounded-lg hover:bg-green-50 transition-colors">
                                    Cek Toko
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="text-xs md:text-base font-bold text-gray-900 mb-2 uppercase tracking-tight">Deskripsi Produk</h3>
                            <div className="prose prose-sm text-gray-600 leading-relaxed whitespace-pre-wrap text-xs sm:text-sm">
            ...
            <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-100 shadow-[0_-8px_24px_rgba(0,0,0,0.06)]">
                <div className="max-w-7xl mx-auto px-4 py-3 md:px-8 md:py-4">
                    ...
                    <div className="flex flex-col gap-2 lg:hidden">
                            ...
                            {product.stock > 0 && (
                                <div className="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-xl px-1 py-1">
                                    <button
                                        ...
                                        disabled={quantity <= 1}
                                        className="h-7 w-7 flex items-center justify-center rounded-lg text-gray-500 hover:text-[#1B5E20] hover:bg-white disabled:opacity-30 transition-all"
                                    >
                        ...
                        {product.stock > 0 ? (
                            <div className="grid grid-cols-4 gap-2">
                                <button
                                    onClick={() => router.back()}
                                    className="col-span-1 py-2 border border-gray-200 text-gray-500 rounded-xl active:scale-95 transition-all flex flex-col items-center justify-center gap-0.5"
                                >
                                    <ArrowLeft className="h-4 w-4" />
                                    <span className="text-[9px] font-medium leading-none">Kembali</span>
                                </button>
                                <button
                                    onClick={handleAddToCart}
                                    className="col-span-1 py-2.5 border border-[#1B5E20] text-[#1B5E20] text-xs font-medium rounded-xl hover:bg-green-50 active:bg-green-100 transition flex items-center justify-center"
                                >
                                ...
                                <WishlistButton
                                    ...
                                    className="col-span-1 h-auto py-2.5 border border-gray-200 rounded-xl flex items-center justify-center"
                                />
                                <button
                                    onClick={handleBuyNow}
                                    className="col-span-1 py-2.5 bg-[#1B5E20] text-white text-xs font-bold rounded-xl hover:bg-green-800 active:bg-green-900 shadow-sm transition"
                                >
                                    Beli
                                </button>
                            </div>
                        ) : (
                            <button disabled className="w-full py-3 bg-gray-100 text-gray-400 text-xs font-bold rounded-xl cursor-not-allowed uppercase tracking-widest">
                                Stok Habis
                            </button>
                        )}
                    </div>
                    ...
                    <div className="hidden lg:flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-3">
                            <button
                                onClick={() => router.back()}
                                className="p-3 border border-gray-200 text-gray-500 rounded-xl hover:bg-gray-50 active:scale-95 transition-all flex items-center justify-center"
                                title="Kembali"
                            >
                            ...
                            {product.stock > 0 && (
                                ...
                                <div className="flex items-center border border-gray-200 rounded-lg bg-gray-50/50">
                                    <button onClick={() => handleQuantityChange('dec')} disabled={quantity <= 1} className="p-3 text-gray-500 hover:text-[#1B5E20] disabled:opacity-30">
                                        <Minus className="h-4 w-4" />
                                    </button>
                                    <span className="w-8 text-center font-bold text-base text-gray-900">{quantity}</span>
                                    <button onClick={() => handleQuantityChange('inc')} disabled={quantity >= product.stock} className="p-3 text-gray-500 hover:text-[#1B5E20] disabled:opacity-30">
                            ...
                            {product.stock > 0 ? (
                                <>
                                    <button onClick={handleAddToCart} className="w-36 py-3 border border-[#1B5E20] text-[#1B5E20] text-sm font-medium rounded-xl hover:bg-green-50 transition flex items-center justify-center space-x-2">
                                        <ShoppingCart className="h-5 w-5" />
                                        <span>Keranjang</span>
                                    </button>
                                    <WishlistButton
                                        product={{ id: product.id, name: product.name, price: product.price, image: product.images[0]?.url || null, shop: { id: product.shop.slug, name: product.shop.name } }}
                                        className="h-[48px] w-[48px] border border-gray-200"
                                    />
                                    <button onClick={handleBuyNow} className="w-44 py-3 bg-[#1B5E20] text-white text-sm font-medium rounded-xl hover:bg-green-800 shadow-sm transition">
                                        Beli Langsung
                                    </button>
                                </>
                            ) : (
                                <button disabled className="w-64 py-3 bg-gray-100 text-gray-400 text-base font-black rounded-xl cursor-not-allowed uppercase tracking-widest">
                                    Stok Habis
                                </button>
                            )}
                        </div>
